...
# Базовые опции less.
# `export -T` связывает строковую переменную $LESS и массив $less:
# это позволяет собирать набор опций как массив, а наружу экспортировать
# обычную переменную окружения.
export -UT LESS less ' '

# Переносим длинные строки в less по словам, а не посреди слова.
(( ${less[(Ie)--wordwrap]} )) ||
    less+=( --wordwrap )

# lesspipe — это внешний препроцессор для less. Он помогает `less` понятнее
# открывать архивы, сжатые файлы и многие нетекстовые форматы.
# Используем pipe-режим через LESSOPEN: так less получает уже подготовленный
# текст без промежуточного временного файла.
